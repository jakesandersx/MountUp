...
import { addAthleteToRoster, getFullRosterBySport, getTeamsBySport, insertNewTeam, updateTeam } from "../mongo/teams";
...
router.patch("/update", isLoggedIn, async(req: Request, res: Response, next: NextFunction) => {
  try{
...
router.post('/addtoroster', isLoggedIn, async(req: Request, res:Response, next: NextFunction) => {
  try{
    const teamId = req.body.teamId;
    const athleteId = req.body.athleteId;

    const updatedTeam = await addAthleteToRoster(teamId, athleteId);
...
router.post('/getroster', isLoggedIn, async(req: Request, res: Response, next: NextFunction) => {
  try{
    const teamId = req.body.teamId;

    const roster = await getFullRosterBySport(teamId);
